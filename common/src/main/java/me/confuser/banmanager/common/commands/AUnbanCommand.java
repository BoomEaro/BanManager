...
public class AUnbanCommand extends CommonCommand {

  public AUnbanCommand(BanManagerPlugin plugin) {
    super(plugin, "aunban", true, UnbanCommandParser.class, 0);
  }
  ...
  public boolean onCommand(final CommonSender sender, CommandParser originalParser) {
    ...
    if (!isBanned) {
      Message message = Message.get("aunban.error.noExists");
    ...
    getPlugin().getScheduler().runAsync(() -> {
      ...
      if (!actor.getUUID().equals(ban.getActor().getUUID()) && !sender.hasPermission("bm.exempt.override.aban")
              && sender.hasPermission("bm.command.aunban.own")) {
        Message.get("aunban.error.notOwn").set("player", ban.getPlayer().getName()).sendTo(sender);
        return;
      }
      ...
      Message message = Message.get("aunban.notify");
      message
      ...
      if (!sender.hasPermission("bm.notify.aunban") || parser.isSilent()) {
        message.sendTo(sender);
      }

      if (!parser.isSilent()) {
        getPlugin().getServer().broadcast(message.toString(), "bm.notify.aunban");
      }
    });
